...
def main():
    ...
    # 3. Setup Animation (Premium Dark Theme)
    DARK = '#111116'
    GRID_C = '#444455'
    ...
    style_ax(axs[0], "GC Plane (XY Plane)", "x (kpc) [Sun is at x=-8.2]", "y (kpc)", 3.8)
    style_ax(axs[1], "Side Profile (XZ Plane)", "x (kpc)", "z (kpc)", 3.8, z_lim=1.8)
    style_ax(axs[2], "Front Profile (YZ Plane)", "y (kpc)", "z (kpc)", 3.8, z_lim=1.8)
    ...
    def update(frame):
        # Calculate current index along each orbit, wrapping around if it exceeds n_orbit_steps
        ...
        # Get coordinates/velocities in the rotating bar frame: shape (n_sample, n_tail, 6)
        pos_trail_bar = coords[np.arange(n_sample)[:, None], trail_indices, :]
        
        # Current bar-frame velocity Vx (state at the current frame corresponds to trail index 0)
        vx_bar_current = pos_trail_bar[:, 0, 3]
        
        # Project past frame states back into GC frame at their respective past animation times
        dt_frame = 0.4 / n_orbit_steps # time per step
        t_trail = (frame - np.arange(n_tail)) * dt_frame
        alpha_t = alpha_0 + omega * t_trail # shape (n_tail,)
        
        cos_a = np.cos(alpha_t)[None, :] # shape (1, n_tail)
        ...
        # Apply unified rotation to all particles to preserve the bar shape!
        ...
        # Fill trails
        x_trail_cyan[:n_cy, :n_tail] = x_gc[cyan_idx, :]
        y_trail_cyan[:n_cy, :n_tail] = y_gc[cyan_idx, :]
        z_trail_cyan[:n_cy, :n_tail] = z_gc[cyan_idx, :]
        
        x_trail_gold[:n_go, :n_tail] = x_gc[gold_idx, :]
        y_trail_gold[:n_go, :n_tail] = y_gc[gold_idx, :]
        z_trail_gold[:n_go, :n_tail] = z_gc[gold_idx, :]
        
        x_trail_magenta[:n_ma, :n_tail] = x_gc[magenta_idx, :]
        y_trail_magenta[:n_ma, :n_tail] = y_gc[magenta_idx, :]
        z_trail_magenta[:n_ma, :n_tail] = z_gc[magenta_idx, :]
        ...
        dots_cyan_xy.set_data(x_gc[cyan_idx, 0], y_gc[cyan_idx, 0])
        dots_cyan_xz.set_data(x_gc[cyan_idx, 0], z_gc[cyan_idx, 0])
        dots_cyan_yz.set_data(y_gc[cyan_idx, 0], z_gc[cyan_idx, 0])
        
        dots_gold_xy.set_data(x_gc[gold_idx, 0], y_gc[gold_idx, 0])
        dots_gold_xz.set_data(x_gc[gold_idx, 0], z_gc[gold_idx, 0])
        dots_gold_yz.set_data(y_gc[gold_idx, 0], z_gc[gold_idx, 0])
        
        dots_magenta_xy.set_data(x_gc[magenta_idx, 0], y_gc[magenta_idx, 0])
        dots_magenta_xz.set_data(x_gc[magenta_idx, 0], z_gc[magenta_idx, 0])
        dots_magenta_yz.set_data(y_gc[magenta_idx, 0], z_gc[magenta_idx, 0])
        
        # Update rotating ellipse patch angle in degrees using the current frame's angle
        alpha_head = alpha_0 + omega * frame * dt_frame
        bar_xy.angle = np.degrees(alpha_head)
