...
def main():
    ...
    p_kosh     = jnp.array(params_all[[i for i, n in enumerate(names)
    ...
    inv_v = BulgeKinematicInverter(grid_axes, solar, n_bins=n_bins_v, kl_weight=0.0, curv_weight=0.0)
    inv_b = BulgeKinematicInverter(grid_axes, solar, n_bins=1,       kl_weight=0.0, curv_weight=0.0)
    inv_g = BulgeKinematicInverter(grid_axes, solar, n_bins=1,       kl_weight=0.0, curv_weight=0.0)

    # Build masked meta axes
    meta_axes_v = {k: (0 if (jnp.ndim(v) > 0 and v.shape[0] == obs_mu_pm_v.shape[0]) else None)
                   for k, v in meta_v.items()}
    n_pix_b = len(meta_b['l'])
    meta_axes_b = {k: (0 if (jnp.ndim(v) > 0 and v.shape[0] == n_pix_b) else None)
                   for k, v in meta_b.items()}
    n_pix_g = len(meta_g['l'])
    meta_axes_g = {k: (0 if (jnp.ndim(v) > 0 and v.shape[0] == n_pix_g) else None)
    ...
    def profile_nll(omega_scalar):
        ...
        # --- VIRAC PM loss ---
        preds_v = vmap(
            lambda m: inv_v.predict_parametric(p_kosh, omega, alpha, m, log_f_bulge=log_f),
            in_axes=(meta_axes_v,)
        )(meta_v)
        ...
        def pm_nll_pixel(pred_mu, pred_cov, obs_mu, obs_cov, obs_err, mask):
            ...
            def bin_nll(k):
                ...
                inv_C = jnp.array([[C[1,1], -C[0,1]], [-C[1,0], C[0,0]]]) / det
                return 0.5 * (jnp.log(det) + d @ inv_C @ d)
            total = jnp.sum(vmap(bin_nll)(jnp.arange(pred_mu.shape[0])))
            return jnp.where(mask, total, 0.0)

        nll_pm = jnp.sum(vmap(pm_nll_pixel)(
            pred_mu_pm_v, pred_cov_pm_v, obs_mu_pm_v, obs_cov_pm_v, obs_mu_err_v, mask_v))

        # --- BRAVA RV loss ---
        preds_b = vmap(
            lambda m: inv_b.predict_parametric(p_kosh, omega, alpha, m, log_f_bulge=log_f),
            in_axes=(meta_axes_b,)
        )(meta_b)
        ...
        nll_rv_b = jnp.sum(vmap(rv_nll)(
            pred_mu_rv_b, pred_var_rv_b, obs_mu_rv_b.flatten(), obs_var_rv_b.flatten(), mask_b))

        # --- GIBS RV loss ---
        preds_g = vmap(
            lambda m: inv_g.predict_parametric(p_kosh, omega, alpha, m, log_f_bulge=log_f),
            in_axes=(meta_axes_g,)
        )(meta_g)
        pred_mu_rv_g  = preds_g[2][:, 0]
        pred_var_rv_g = preds_g[3][:, 0]

        nll_rv_g = jnp.sum(vmap(rv_nll)(
            pred_mu_rv_g, pred_var_rv_g, obs_mu_rv_g.flatten(), obs_var_rv_g.flatten(), mask_g))
    ...
    # Use only points within delta_NLL < 5 for the fit
    close = delta_nll < 5.0
    if close.sum() < 3:
    ...
    # 1-sigma: NLL increases by 0.5 from minimum → sigma = 1/sqrt(2a)
    ...
    print(f"  (replace the current 'omega_p = 0.5 * ((omega - 45.0) / 15.0)**2' in")
    print(f"   jax_kinematic_inverter.py, using omega_param from the saved results)")

    import matplotlib.pyplot as plt
    fig, ax = plt.subplots(figsize=(8, 5))
    ax.plot(omega_scan, delta_nll, 'o-', color='#4488ff', lw=1.5, ms=4, label='Profile NLL')
    omega_fine = np.linspace(omega_scan[close][0], omega_scan[close][-1], 200)
    ax.plot(omega_fine, np.polyval(coeffs, omega_fine) - nll_scan.min(),
            '--', color='#ff9944', lw=2, label=f'Parabola fit')
    ax.axvline(omega_best, color='white', linestyle=':', lw=1.0, label=f'Best fit Ω={omega_best:.2f}')
    ax.axvline(omega_parabola, color='#ff4488', linestyle='--', lw=1.5,
               label=f'Parabola min Ω={omega_parabola:.2f}')
    ax.axhline(0.5, color='#88ff88', linestyle=':', lw=1.0, label='1σ (ΔNLL=0.5)')
    ax.fill_betweenx([0, 0.5],
                     omega_parabola - sigma_omega, omega_parabola + sigma_omega,
                     alpha=0.15, color='#88ff88')
    ax.set_xlabel(r'$\Omega_{\rm bar}$ [km/s/kpc]', color='white')
    ax.set_ylabel(r'$\Delta \mathcal{L}$ (profile NLL)', color='white')
    ax.set_title(f'Profile Likelihood for Pattern Speed\n'
