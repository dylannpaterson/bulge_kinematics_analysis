...
def main():
    ...
    # f_bulge is fixed from the SynthPop model to preserve star-count normalisation.
    # Floating it during the kinematic fit would corrupt the bulge/disk density ratio
    # when results are fed back into SynthPop.
    ...
    def get_masked_parametric_nll(inverter, params, meta_batch, obs_mu_pm=None, obs_cov_pm=None, obs_mu_err_pm=None, obs_mu_rv=None, obs_var_rv=None, obs_mu_err_rv=None, mask=None, rv_weight=1.0):
        ...
        preds = vmap(lambda m: inverter.predict_parametric(p_kosh, omega, alpha_r, m, log_f_bulge=log_f_bulge), in_axes=(meta_axes,))(flat_meta)
    ...
    def combined_loss(params):
        p_kosh = params[:18]
        omega = params[18]
        # f_bulge is fixed from SynthPop — not a free parameter
        alpha_d = fixed_alpha
        loss_v = get_masked_parametric_nll(inverter_v, (p_kosh, omega, alpha_d, fixed_log_f_bulge), meta_v, obs_mu_pm=obs_mu_pm_v, obs_cov_pm=obs_cov_pm_v, obs_mu_err_pm=obs_mu_err_v, mask=mask_v)
        loss_b = get_masked_parametric_nll(inverter_b, (p_kosh, omega, alpha_d, fixed_log_f_bulge), meta_b, obs_mu_rv=obs_mu_rv_b, obs_var_rv=obs_var_rv_b, obs_mu_err_rv=obs_mu_err_b, mask=mask_b)
        loss_g = get_masked_parametric_nll(inverter_g, (p_kosh, omega, alpha_d, fixed_log_f_bulge), meta_g, obs_mu_rv=obs_mu_rv_g, obs_var_rv=obs_var_rv_g, obs_mu_err_rv=obs_mu_err_g, mask=mask_g)
    ...
    # 4. Optimization
    p_init = np.array([
        v0_init, y0_init,             # v0_str, y0_str
        s_x0, s_x1, s_y0, s_y1, s_z0, s_z1, # sigmas
        h0_r[0], h0_r[1], h0_r[2],    # h0_r
        h0_z[0], h0_z[1], h0_z[2],    # h0_z
        C_par_r, C_perp_r, C_par_z, C_perp_z, # C params
        omega_init,                   # omega
    ])
    
    bounds = [
        (0, 400), (0.01, 10.0),       # v0_str, y0_str
        (10, 400), (0, 600), (10, 400), (0, 600), (10, 400), (0, 600), # sigmas
        (0.1, 15.0), (0.1, 15.0), (0.1, 15.0), # h0_r
        (0.1, 15.0), (0.1, 15.0), (0.1, 15.0), # h0_z
        (0.5, 10.0), (0.5, 20.0), (0.5, 10.0), (0.5, 20.0), # C params
        (0, 150),                      # omega
    ]
    
    p_names = [
        'v0_str', 'y0_str', 'sig_i0_x', 'sig_i1_x', 'sig_i0_y', 'sig_i1_y', 'sig_i0_z', 'sig_i1_z',
        'h0_rx', 'h0_ry', 'h0_rz', 'h0_zx', 'h0_zy', 'h0_zz',
        'C_par_r', 'C_perp_r', 'C_par_z', 'C_perp_z',
        'omega'
    ]

    print(f"\n--- Combined REAL DATA Parametric Optimization (H25C20) ---")
    ...
    print(f"f_bulge fixed at: {raw_f_bulge:.4f} (logit={fixed_log_f_bulge:.4f}) — not a free parameter")
    
    def scipy_obj(x):
        l, g = val_and_grad_fn(jnp.array(x))
        if not hasattr(scipy_obj, "n"): scipy_obj.n = 0
        if scipy_obj.n % 10 == 0:
            print(f"  Iter {scipy_obj.n:3d}: Loss={l:12.4f}, v0={x[0]:.2f}, omega={x[18]:.2f}, f_bulge={raw_f_bulge:.4f} (fixed)")
        scipy_obj.n += 1
        return float(l), np.array(g).astype(np.float64)

    max_iter_val = 5 if os.environ.get("FAST_TEST") == "1" else 10000
    res = minimize(scipy_obj, p_init, method='L-BFGS-B', jac=True, bounds=bounds, options={'maxiter': max_iter_val, 'ftol': 1e-12, 'gtol': 1e-12})
    print("\nOptimization Complete.")
    for i, name in enumerate(p_names): print(f"{name:<15} | {res.x[i]:10.2f}")
    
    output_path = os.path.join(project_root, "results/inversion/parametric_fit_results_real_h25c20.npz")
    # Store the fixed f_bulge alongside fitted params so downstream code can reconstruct correctly
