...
def get_bulge_density_model(model_name="Besancon_Coleman2020_symmetric"):
    config = {
    ...
    def bulge_only_density(pos):
        """Specifically extracts populations named 'bulge' in Msun/kpc^3."""
...
def interpolate_velocity_field(grid_params, axes_x, axes_y, axes_z, X, Y, Z):
    """
    Interpolates ux and uy on a 3D grid using triaxial symmetries.
    """
...
def main():
    ...
    # Flux components: J = rho * u
    Jx = rho * ux
    Jy = rho * uy
    Jz = rho * uz
    
    # Finite differences on regular grid
    ...
    print(f"Median Absolute Divergence: {np.nanmedian(np.abs(div_J)):.4f} Msun/kpc^3/yr")
    print(f"Relative Divergence Error (div / typical_flux_gradient): {np.nanmedian(np.abs(rel_error))*100:.2f}%")
    ...
    # XY Plane projection (Z-averaged or mid-plane z=0)
    z_mid = nz // 2
    ...
    # Colormap: diverging red-blue or viridis style. Let's use seismic for divergence to show source/sink balance
    div_lim = 1.5 * np.nanpercentile(np.abs(div_J[:, :, z_mid]), 90)
    im0 = ax0.pcolormesh(x_grid, y_grid, div_J[:, :, z_mid].T, cmap='coolwarm', vmin=-div_lim, vmax=div_lim, shading='auto')
    fig.colorbar(im0, ax=ax0, label=r'$\nabla \cdot (\rho \mathbf{u})$ [$M_\odot/{\rm kpc}^3/{\rm yr}$]')
    
    # Overplot streamplot of mass flux J
    ax0.streamplot(x_grid, y_grid, Jx[:, :, z_mid].T, Jy[:, :, z_mid].T, color='white', linewidth=1.0, density=1.2, arrowstyle='->')
    
    ax0.set_title("Midplane Mass Flux & Divergence (XY Plane)\nOverlaid with Mass Flux Streamlines", fontsize=12, fontweight='bold', pad=12)
    ...
    ax1.hist(rel_error.flatten(), bins=100, range=(-2.0, 2.0), color='cyan', alpha=0.7, edgecolor=DARK, density=True)
    ax1.axvline(0, color='red', linestyle='--')
    ax1.set_title("Divergence Error Distribution\n(Normalized by Typical Flux Gradient Scale)", fontsize=12, fontweight='bold', pad=12)
    ax1.set_xlabel(r"$\nabla \cdot (\rho \mathbf{u}) / |\nabla (\rho |\mathbf{u}|)|$")
    ax1.set_ylabel("Probability Density")
    
    plt.suptitle("Continuity Equation Conservation Check of the Non-Parametric Model", fontsize=15, fontweight='bold', y=0.98)
