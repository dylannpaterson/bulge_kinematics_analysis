...
def main():
    ...
    if not os.path.exists(fit_path):
        raise FileNotFoundError(f"Non-parametric fit results not found at {fit_path}. Please run the fit script first.")
    ...
    print(f"Pattern Speed Omega: {omega:.2f} km/s/kpc, Bar Angle: {alpha_deg:.2f} deg, f_bulge: {raw_f_bulge:.2f}")
    ...
    # --- VIRAC2 (PMs) ---
    print("Loading aligned VIRAC2 real data...")
    ...
    pred_v = vmap(lambda m: inverter_v.predict_pixel(grid_params, omega, m, log_f_bulge=log_f_bulge, return_components=True), in_axes=(meta_axes,))(meta_v)
    ...
    obs_err_rv_b = real_b['mu_err'].flatten() if 'mu_err' in real_b else np.ones_like(obs_mu_rv_b) * 10.0
    ...
    pred_b = vmap(lambda m: inverter_b.predict_pixel(grid_params, omega, m, log_f_bulge=log_f_bulge), in_axes=(meta_axes_b,))(meta_b)
    pred_mu_rv_b = np.array(pred_b[2])[:, 0].flatten() # (N_pix,)

    # --- GIBS (RVs) ---
    print("Loading GIBS real data...")
    ...
    obs_err_rv_g = real_g['mu_err'].flatten() if 'mu_err' in real_g else np.ones_like(obs_mu_rv_g) * 10.0
    ...
    pred_g = vmap(lambda m: inverter_g.predict_pixel(grid_params, omega, m, log_f_bulge=log_f_bulge), in_axes=(meta_axes_g,))(meta_g)
    ...
    def plot_component(comp_idx, comp_name, filename):
        ...
        for i, k_idx in enumerate(k_indices):
            ...
            # Row 2: NORMALIZED RESIDUAL (Model - Obs) / Error
            ax_res = axes[2, i]
            res = (model_data - obs_data) / (obs_err + 1e-10)
            res_masked = np.where(valid_mask & m & (obs_err < 1.0), res, np.nan)
            
            im_res = ax_res.pcolormesh(L_grid, B_grid, res_masked, cmap='RdBu_r', shading='auto', vmin=-5, vmax=5)
        ...
        plt.suptitle(fr"H25C20 Real Data Non-Parametric Fit: Proper Motion $\mu_{comp_name}$", fontsize=16)
    ...
    def plot_covariance_metric(metric_type, metric_name, filename):
        ...
        plt.suptitle(fr"H25C20 Real Data Non-Parametric Fit: Proper Motion {metric_name}", fontsize=16)
        plt.tight_layout(rect=[0, 0.03, 1, 0.95])
        plt.savefig(filename, dpi=200)
        plt.close()
        print(f"Saved {filename}")
    ...
    plot_covariance_metric('sig_l', 'Dispersion l', os.path.join(out_dir, "non_parametric_fit_sigma_l_h25c20.png"))
    plot_covariance_metric('sig_b', 'Dispersion b', os.path.join(out_dir, "non_parametric_fit_sigma_b_h25c20.png"))
    plot_covariance_metric('corr', 'Correlation', os.path.join(out_dir, "non_parametric_fit_correlation_h25c20.png"))
    ...
    plt.subplot(1, 2, 1)
    plt.hist(norm_res_b, bins=15, range=(-5, 5), color='crimson', edgecolor='black', alpha=0.7)
    plt.axvline(0, color='black', linestyle='--', linewidth=1.5)
    plt.xlabel(r'$v_{los}$ Norm. Residual (Obs - Model) / $\sigma_{obs}$', fontsize=10)
    plt.ylabel('Count', fontsize=10)
    plt.title(f'BRAVA RV Norm. Residuals (N={len(res_b)})', fontsize=11, fontweight='bold')
    plt.grid(True, alpha=0.3)
    
    plt.subplot(1, 2, 2)
    plt.hist(norm_res_g, bins=15, range=(-5, 5), color='royalblue', edgecolor='black', alpha=0.7)
    plt.axvline(0, color='black', linestyle='--', linewidth=1.5)
    plt.xlabel(r'$v_{los}$ Norm. Residual (Obs - Model) / $\sigma_{obs}$', fontsize=10)
    plt.ylabel('Count', fontsize=10)
    plt.title(f'GIBS RV Norm. Residuals (N={len(res_g)})', fontsize=11, fontweight='bold')
    plt.grid(True, alpha=0.3)
    
    plt.suptitle("H25C20 Real Data Non-Parametric Fit: Radial Velocity Norm. Residuals", fontsize=13, fontweight='bold')
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "non_parametric_fit_rv_residuals_h25c20.png"), dpi=200)
    plt.close()
    print(f"Saved {os.path.join(out_dir, 'non_parametric_fit_rv_residuals_h25c20.png')}")
