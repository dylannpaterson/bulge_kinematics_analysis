"""
Galactocentric bulk motion comparison between parametric and non-parametric fits.

Reconstructs the full 3-D bulk velocity field in the Galactocentric frame
for both the parametric and non-parametric fits, projecting the velocity vector fields
onto the GC XY, XZ, and YZ planes for side-by-side diagnostic plotting.
"""
...
# ---------------------------------------------------------------------------
# Kinematic Reconstruction Helpers
# ---------------------------------------------------------------------------

def compute_parametric_bar(
    v0_str: float, y0_str: float, omega: float, X_bar: np.ndarray, Y_bar: np.ndarray
...
def bar_to_gc(
    vx_bar: np.ndarray, vy_bar: np.ndarray, alpha_rad: float
) -> tuple[np.ndarray, np.ndarray]:
    """
    Rotate bar-frame velocities into GC frame.

    Parameters
    ----------
    vx_bar, vy_bar : np.ndarray
    alpha_rad : float

    Returns
    -------
    vx_gc, vy_gc : np.ndarray
    """
...
def compute_parametric_gc(
    ...
    """
    Compute parametric model velocities in the Galactocentric frame.
...
def compute_nonparametric_gc(
    ...
    """
    Compute non-parametric grid velocities in the Galactocentric frame.
    ...
    Returns
    -------
    vx_gc_all, vy_gc_all : np.ndarray
    """
    ...
    for iz in range(NZ):
        ...
        vx_gc_all[:, :, iz], vy_gc_all[:, :, iz] = bar_to_gc(
            vx_total_bar, vy_total_bar, alpha_rad
        )
...
def project_planes(
    ...
    """Project full 3D velocities onto 2D slices."""
    return (
        np.nanmean(VX, axis=2),
        np.nanmean(VY, axis=2),
        np.nanmean(VX, axis=1),
        np.nanmean(VY, axis=1),
        np.nanmean(VX, axis=0),
        np.nanmean(VY, axis=0),
    )


def interpolate_to_gc_grid(
    vx_gc_all: np.ndarray,
    vy_gc_all: np.ndarray,
    X_gc: np.ndarray,
    Y_gc: np.ndarray,
    xi: np.ndarray,
    yi: np.ndarray,
    ...
    """Interpolate rotated bar-frame velocities onto a uniform GC grid."""
    n_gc = len(xi)
    ...
    for k in range(NZ):
        VXI[:, :, k] = griddata(
            pts, vx_gc_all[:, :, k].flatten(), (grid_xi, grid_yi), method="cubic"
        )
        VYI[:, :, k] = griddata(
            pts, vy_gc_all[:, :, k].flatten(), (grid_xi, grid_yi), method="cubic"
        )
...
def style_ax(ax: plt.Axes) -> None:
    """Apply styling configurations to a Matplotlib Axes object."""
...
def main() -> None:
    """Execute GC-frame bulk motion comparison."""
    ...
    # GC coords of bar-frame source points
    ...
    vx_p, vy_p = compute_parametric_gc(
        v0_str, y0_str, omega_p, alpha_rad, X_bar, Y_bar
    )
    VXP = np.broadcast_to(vx_p[:, :, None], (n_dense, n_dense, NZ)).copy()
    VYP = np.broadcast_to(vy_p[:, :, None], (n_dense, n_dense, NZ)).copy()
    VXI_p, VYI_p, grid_xi, grid_yi = interpolate_to_gc_grid(
        VXP, VYP, X_gc, Y_gc, xi, yi, NZ
    )
    VXI_p_full = mirror_z(VXI_p)
    VYI_p_full = mirror_z(VYI_p)
    vx_xy_p, vy_xy_p, vx_xz_p, _, vx_yz_p, vy_yz_p = project_planes(
        VXI_p_full, VYI_p_full
    )

    # ── Non-parametric GC field ──────────────────────────
    print("Computing non-parametric GC velocities...")
    vx_n_bar, vy_n_bar = compute_nonparametric_gc(
        grid_params, omega_np, alpha_rad, axes_x, axes_y, axes_z, X_bar, Y_bar
    )
    VXI_n, VYI_n, _, _ = interpolate_to_gc_grid(
        vx_n_bar, vy_n_bar, X_gc, Y_gc, xi, yi, NZ
    )
    VXI_n_full = mirror_z(VXI_n)
    VYI_n_full = mirror_z(VYI_n)
    vx_xy_n, vy_xy_n, vx_xz_n, _, vx_yz_n, vy_yz_n = project_planes(
        VXI_n_full, VYI_n_full
    )
    ...
    for row, (label, vx_xy, vy_xy, vx_xz, vx_yz, vy_yz) in enumerate(row_data):
        ...
        ax.plot(
            r_bar * np.cos(alpha_rad),
            -r_bar * np.sin(alpha_rad),
            color="#ff9944",
            linestyle="--",
            linewidth=1.3,
            alpha=0.7,
        ...
        ax.quiver(
            grid_x_xz[::skip, ::skip_z],
            grid_z_xz[::skip, ::skip_z],
            vx_xz[::skip, ::skip_z],
            np.zeros_like(vx_xz[::skip, ::skip_z]),
            color="white",
            scale=500,
            width=0.003,
            headwidth=4,
        ...
        ax.quiver(
            grid_y_yz[::skip, ::skip_z],
            grid_z_yz[::skip, ::skip_z],
            vy_yz[::skip, ::skip_z],
            np.zeros_like(vy_yz[::skip, ::skip_z]),
            color="white",
            scale=300,
            width=0.003,
            headwidth=4,
    ...
    # Column titles
    axs[0, 0].set_title(
        "Galactic Plane View  (Z-Averaged)\n"
    ...
    axs[0, 1].set_title(
        "Side Profile  ($y_{\\rm GC}$-Averaged)\n"
    ...
    axs[0, 2].set_title(
        "Front Profile  ($x_{\\rm GC}$-Averaged)\n"
    ...
    cb.set_label(
        "Stellar Bulk Velocity Magnitude [km/s]",
        color="white",
        fontsize=10,
        labelpad=10,
    )
