"""
Bar-frame bulk motion comparison between parametric and non-parametric fits.

Reconstructs the full 3-D bulk velocity field in the bar-aligned rotating frame
for both the parametric and non-parametric fits, projecting the velocity vector fields
onto the XY, XZ, and YZ planes for side-by-side diagnostic plotting.
"""
...
def compute_nonparametric_bar(
    ...
    """
    Reconstruct full 4-quadrant velocity field in the bar frame from the grid.

    Uses bilinear interpolation and symmetry rules matching BulgeKinematicInverter.
    ...
    Returns
    -------
    vx_all, vy_all : np.ndarray of shape (n_dense, n_dense, NZ)
    """
...
def panel_xy(
    ...
    """Plot the horizontal plane bulk motion panel."""
    v_mag = np.sqrt(vx**2 + vy**2)
    im = ax.pcolormesh(
        X, Y, v_mag, cmap=CMAP, shading="auto", vmin=0, vmax=VMAX, alpha=0.85
    )
    ax.streamplot(
        x_dense,
        y_dense,
...
def panel_xz(
    ...
    grid_z: np.ndarray,
    vx: np.ndarray,
    axes_z_full: np.ndarray,
    skip: int,
    skip_z: int,
) -> plt.image.AxesImage:
    """Plot the X-Z plane bulk motion panel."""
    im = ax.pcolormesh(
        grid_x,
        grid_z,
        np.sqrt(vx**2),
...
def panel_yz(
    ...
    """Plot the Y-Z plane bulk motion panel."""
    im = ax.pcolormesh(
        grid_y,
        grid_z,
        np.sqrt(vy**2),
...
def main() -> None:
    """Execute bulk motion bar comparison visualization."""
    ...
    vx_xy_p, vy_xy_p, vx_xz_p, vy_xz_p, vx_yz_p, vy_yz_p = project_planes(
        VXP_full, VYP_full
    )

    # --- Non-parametric velocities ---
    print("Computing non-parametric model velocities...")
    ...
    vx_xy_n, vy_xy_n, vx_xz_n, vy_xz_n, vx_yz_n, vy_yz_n = project_planes(
        VXN_full, VYN_full
    )
    ...
    last_im = None
    for row, (label, (vxx, vyy, vxz, _, _, vyyz)) in enumerate(
        zip(row_labels, data_rows)
    ):
        for ax in axs[row]:
            style_ax(ax)
        ...
        # Col 0: XY bar plane
        im = panel_xy(
            axs[row, 0], X_bar, Y_bar, vxx, vyy, x_dense, y_dense, alpha_deg, skip
        )
        ...
        # Col 1: XZ side
        im = panel_xz(
            axs[row, 1], grid_x_xz, grid_z_xz, vxz, axes_z_full, skip, skip_z
        )
    ...
    axs[0, 0].set_title(
        "Bar Plane View  (Z-Averaged)\n" r"$|v|$ with streamlines",
        color="white",
        fontsize=12,
        fontweight="bold",
    )
    axs[0, 1].set_title(
        "Side Profile  ($y$-Averaged)\n" r"Streaming $v_x$ along Bar Major Axis",
        color="white",
        fontsize=12,
        fontweight="bold",
    )
    axs[0, 2].set_title(
        "Front Profile  ($x$-Averaged)\n" r"Rotation $v_y$ $\perp$ Bar",
        color="white",
        fontsize=12,
        fontweight="bold",
    )
