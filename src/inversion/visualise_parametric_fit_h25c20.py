"""
Visualization diagnostics for the parametric kinematic fit.

Generates binned proper-motion component maps, dispersion maps, correlation maps, and
radial velocity normalized residual histograms, comparing VVV, BRAVA, and GIBS
observational datasets against the optimized parametric kinematic model.
"""
...
def main() -> None:
    """Generate diagnostic maps and plots for the parametric fit."""
    ...
    # Robustly load parameter values based on names
    if "names" in res:
        ...
        p_kosh_indices = [
            ...
            if name not in ["omega", "alpha", "f_bulge", "sigmoid_f_bulge"]
        ]
    ...
    print(
        f"Omega: {omega:.2f}, Alpha: {alpha_deg:.2f} deg, Log(f_bulge): {log_f_bulge:.2f}"
    )
    ...
    pred_v = vmap(
        lambda m: inverter_v.predict_parametric(
            p_kosh, omega, alpha_rad, m, log_f_bulge=log_f_bulge, return_components=True
        ),
        in_axes=(meta_axes,),
    )(meta_v)
    ...
    pred_b = vmap(
        lambda m: inverter_b.predict_parametric(
            p_kosh, omega, alpha_rad, m, log_f_bulge=log_f_bulge
        ),
        in_axes=(meta_axes_b,),
    )(meta_b)
    ...
    pred_g = vmap(
        lambda m: inverter_g.predict_parametric(
            p_kosh, omega, alpha_rad, m, log_f_bulge=log_f_bulge
        ),
        in_axes=(meta_axes_g,),
    )(meta_g)
    ...
    def plot_component(comp_idx, comp_name, filename):
        ...
        for i, k_idx in enumerate(k_indices):
            ...
            im_res = ax_res.pcolormesh(
                L_grid, B_grid, res_masked, cmap="RdBu_r", vmin=-5, vmax=5, shading="auto"
            )
        ...
        plt.suptitle(
            fr"H25C20 Observed Data Parametric Fit: Proper Motion $\mu_{comp_name}$",
            fontsize=16,
        )
    ...
    def plot_covariance_metric(metric_type, metric_name, filename):
        ...
        for i, k_idx in enumerate(k_indices):
            ...
            res_for_limits = np.where(
                valid_for_limits, model_data - obs_data, np.nan
            )
        ...
        plt.suptitle(
            fr"H25C20 Observed Data Parametric Fit: {metric_name}", fontsize=16
        )
        plt.tight_layout(rect=[0, 0.03, 1, 0.95])
        plt.savefig(filename, dpi=200)
        print(f"Saved {filename}")
    ...
    plot_component(
        0, "l", os.path.join(out_dir, "parametric_fit_obs_h25c20_pml.png")
    )
    plot_component(
        1, "b", os.path.join(out_dir, "parametric_fit_obs_h25c20_pmb.png")
    )

    print("Generating proper motion dispersion and correlation plots...")
    plot_covariance_metric(
        "sig_l",
        r"Proper Motion Dispersion in Longitude $\sigma_{\mu_l}$",
        os.path.join(out_dir, "parametric_fit_obs_h25c20_sigl.png"),
    )
    plot_covariance_metric(
        "sig_b",
        r"Proper Motion Dispersion in Latitude $\sigma_{\mu_b}$",
        os.path.join(out_dir, "parametric_fit_obs_h25c20_sigb.png"),
    )
    plot_covariance_metric(
        "corr",
        r"Proper Motion Correlation $\rho_{\mu_l, \mu_b}$",
        os.path.join(out_dir, "parametric_fit_obs_h25c20_corr.png"),
    )
    ...
    if len(mask_b) == len(pred_mu_rv_b):
        ...
        err_b = (
            real_b["mu_err"].flatten()[mask_b] if "mu_err" in real_b else 10.0
        )
    ...
    if len(mask_g) == len(pred_mu_rv_g):
        ...
        err_g = (
            real_g["mu_err"].flatten()[mask_g] if "mu_err" in real_g else 10.0
        )
