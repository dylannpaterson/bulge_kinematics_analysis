...
def main():
    # 0. Dynamically Extract Parameters from our Saved Parametric Fit (No SynthPop Loading!)
    param_path = os.path.join(project_root, "results/inversion/parametric_fit_results_real_h25c20.npz")
    if not os.path.exists(param_path):
        raise FileNotFoundError(f"Parametric fit results not found at {param_path}. Please run fit_real_parametric_h25c20.py first.")
    ...
    # 1. Load Aligned Real Data
    print("\nLoading real data...")
    # --- VIRAC2 ---
    data_v = np.load(os.path.join(project_root, 'results/reduction/virac2_real_aligned_h25c20.npz'))
    obs_mu_pm_v = jnp.array(data_v['mu_pm'])
    obs_cov_pm_v = jnp.array(data_v['cov_pm']) + jnp.eye(2)[None, None, :, :]*(0.01**2)
    obs_mu_err_v = jnp.array(data_v['mu_err'])
    mask_v = jnp.array(data_v['mask'])
    b_v_arr = data_v['b']
    mask_v = mask_v & (jnp.abs(b_v_arr) >= 2.0)
    
    with open(os.path.join(project_root, 'results/inversion/huston25c20_metadata_science_binned.pkl'), 'rb') as f:
        meta_v = pickle.load(f)
    n_bins_v = obs_mu_pm_v.shape[1]

    # --- BRAVA ---
    data_b = np.load(os.path.join(project_root, 'results/reduction/brava_real_aligned_h25c20.npz'))
    ...
    with open(os.path.join(project_root, 'results/inversion/huston25c20_brava_real_metadata.pkl'), 'rb') as f:
        meta_b = pickle.load(f)

    # --- GIBS ---
    data_g = np.load(os.path.join(project_root, 'results/reduction/gibs_real_aligned_h25c20.npz'))
    ...
    with open(os.path.join(project_root, 'results/inversion/huston25c20_gibs_metadata_science.pkl'), 'rb') as f:
        meta_g = pickle.load(f)

    # 2. Setup Inverters (kl_weight=0 to bypass default isotropic penalty)
    grid_axes = get_inverter_grid_axes()
    inverter_v = BulgeKinematicInverter(grid_axes, solar, n_bins=n_bins_v, kl_weight=0.0, curv_weight=1.0)
    inverter_b = BulgeKinematicInverter(grid_axes, solar, n_bins=1, kl_weight=0.0, curv_weight=1.0)
    inverter_g = BulgeKinematicInverter(grid_axes, solar, n_bins=1, kl_weight=0.0, curv_weight=1.0)
    ...
    # Streaming velocity prior in first octant (Y >= 0, X >= 0)
    ...
    def custom_kl_divergence_penalty(grid_params):
        ux, uy, log_L11, L21, log_L22, log_L33 = (
            ...
            grid_params[..., 3], grid_params[..., 4], grid_params[..., 5]
        )
        ...
        tr_term = L11_sq / (sig_x_prior**2) + (L21_sq + L22_sq) / (sig_y_prior**2) + L33_sq / (sig_z_prior**2)
    ...
    def get_masked_grid_nll(inverter, grid_params, omega, meta_batch, obs_mu_pm=None, obs_cov_pm=None, obs_mu_err_pm=None, obs_mu_rv=None, obs_var_rv=None, obs_mu_err_rv=None, mask=None, rv_weight=1.0):
        ...
        preds = vmap(lambda m: inverter.predict_pixel(grid_params, omega, m, log_f_bulge=fixed_log_f_bulge), in_axes=(meta_axes,))(flat_meta)
    ...
    def joint_loss_fn(params):
        ...
        # Proper motion term
        nll_pm = get_masked_grid_nll(inverter_v, grid_params, omega, meta_v, obs_mu_pm=obs_mu_pm_v, obs_cov_pm=obs_cov_pm_v, obs_mu_err_pm=obs_mu_err_v, mask=mask_v)
        
        # BRAVA RV term
        nll_rv_b = get_masked_grid_nll(inverter_b, grid_params, omega, meta_b, obs_mu_rv=obs_mu_rv_b, obs_var_rv=obs_var_rv_b, obs_mu_err_rv=obs_mu_err_b, mask=mask_b)
        
        # GIBS RV term
        nll_rv_g = get_masked_grid_nll(inverter_g, grid_params, omega, meta_g, obs_mu_rv=obs_mu_rv_g, obs_var_rv=obs_var_rv_g, obs_mu_err_rv=obs_mu_err_g, mask=mask_g)
        
        # Curvature Regularization (Computed on grid_params)
        curv_reg = 0.0
        for p in range(6):
            field = grid_params[..., p]
        ...
        total_loss = nll_pm + nll_rv_b + nll_rv_g + kl_weight * kl + curv_weight * curv_reg + om_pen
        return total_loss, (nll_pm, nll_rv_b, nll_rv_g, kl_weight * kl, curv_weight * curv_reg, om_pen)
    ...
    current_grid[..., 3] = np.zeros(inverter_v.shape)  # L21 = 0 (diagonal ellipsoid prior)
    ...
    current_grid[..., 3] += np.array(jax.random.normal(k4, inverter_v.shape) * 0.05) # small non-zero starting L21

    initial_omega = 45.0  # displaced from the fitted 37.67 to actively show optimizer movement

    print(f"\n--- Combined REAL DATA Non-Parametric Optimization: VIRAC ({np.sum(mask_v)} pix) + BRAVA ({np.sum(mask_b)} pix) + GIBS ({np.sum(mask_g)} pix) ---")
    ...
    def scipy_objective(x_flat):
        ...
        if not hasattr(scipy_objective, "n_calls"): scipy_objective.n_calls = 0
        if scipy_objective.n_calls % 10 == 0:
            print(f"  Iter {scipy_objective.n_calls:4d}: Loss = {loss:.4f} [PM_V={nll_pm:.2f}, RV_B={nll_rv_b:.2f}, RV_G={nll_rv_g:.2f}, KL={kl:.2f}, Curv={curv:.2f}, OmP={om_pen:.2f}], Om={omega:.2f}")
        scipy_objective.n_calls += 1
        
        return float(loss), np.concatenate([np.array(grid_grads).flatten(), [float(omega_grad)]]).astype(np.float64)

    # 8. Define bounds
    x0 = np.concatenate([current_grid.flatten(), [initial_omega]])
    
    grid_bounds = [(-400, 400), (-400, 400), (1.6, 6.0), (-400, 400), (1.6, 6.0), (1.6, 6.0)] * np.prod(grid_shape[:-1])
    ...
    res = minimize(scipy_objective, x0, method='L-BFGS-B', jac=True, bounds=bounds, options={'maxiter': max_iter_val, 'ftol': 1e-9, 'gtol': 1e-9})
    ...
    output_path = os.path.join(project_root, 'results/inversion/fit_results_real_combined_h25c20.npz')
    np.savez(output_path, grid=final_params, omega=final_omega, 
             axes_x=grid_axes[0], axes_y=grid_axes[1], axes_z=grid_axes[2],
