"""
MCMC analysis of bulge density symmetries to fit bar orientation parameters.

Fits the spatial offsets and bar orientation angle (alpha) of the bulge model using
Markov Chain Monte Carlo (MCMC) over an 8-fold symmetric probe grid in the bar frame.
"""
...
bar_frame_points = (
    probes_first_octant[:, np.newaxis, :] * combinations[np.newaxis, :, :]
).reshape(-1, 3)
...
def get_density_at_params(params: np.ndarray) -> np.ndarray:
    """
    Evaluate the bulge density at 8-fold symmetric points for given model parameters.

    Parameters
    ----------
    params : np.ndarray
        Model parameters ``[x0, y0, z0, alpha]`` where offsets are in kpc and
        alpha is in radians.

    Returns
    -------
    density_grid : np.ndarray
        Reshaped density array of shape (N_probes, 8).
    """
...
def log_likelihood(params: np.ndarray) -> float:
    """
    Compute log-likelihood based on density symmetry deviations.

    Parameters
    ----------
    params : np.ndarray
        Model parameters ``[x0, y0, z0, alpha]``.

    Returns
    -------
    log_lik : float
        Log-likelihood value. Returns -inf for out-of-bound values.
    """
    x0, y0, z0, alpha = params
...
def main() -> None:
    """Run the symmetry MCMC fitting analysis."""
    n_dim = 4
    is_fast = os.environ.get("FAST_TEST") == "1"
    n_walkers = 8 if is_fast else 48
    n_steps = 5 if is_fast else 5000
    ...
    pos = [initial_guess + 1e-2 * np.random.randn(n_dim) for _ in range(n_walkers)]
    ...
    fig = corner.corner(
        samples_plot, labels=labels_plot, truths=medians, show_titles=True, title_fmt=".3f"
    )
